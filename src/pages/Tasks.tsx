  ...
  return (
    <PageLayout title="Tasks">
      <div className="grid gap-6 xl:grid-cols-[1.5fr_1fr]">
        <div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">Create, edit, delete, and move tasks across columns.</p>
            </div>
          </div>
        ...
        <div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
          <h2 className="text-xl font-semibold text-slate-900">Board status</h2>
          <p className="mt-2 text-sm text-slate-500">Drag and drop is not enabled yet; use the buttons to move tasks.</p>
          <div className="mt-6 space-y-4">
            <div className="rounded-3xl bg-slate-100 p-4">
              <p className="text-sm font-semibold text-slate-900">Todo</p>
              <p className="mt-1 text-sm text-slate-600">Tasks that need to start.</p>
            </div>
            <div className="rounded-3xl bg-slate-100 p-4">
              <p className="text-sm font-semibold text-slate-900">In Progress</p>
              <p className="mt-1 text-sm text-slate-600">Tasks you are actively working on.</p>
            </div>
            <div className="rounded-3xl bg-slate-100 p-4">
              <p className="text-sm font-semibold text-slate-900">Completed</p>
              <p className="mt-1 text-sm text-slate-600">Tasks finished and synced with your API.</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <section className="mt-8 grid gap-4 xl:grid-cols-3">
        {(["todo", "in_progress", "completed"] as TaskStatus[]).map((columnKey) => {
          ...
          return (
            <div key={columnKey} className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm">
              <h3 className="text-lg font-semibold text-slate-900">{title}</h3>
              <p className="mt-1 text-sm text-slate-500">{columnTasks.length} task{columnTasks.length === 1 ? "" : "s"}</p>
                    ...
                    <div key={task.id} className="rounded-3xl border border-slate-200 bg-slate-50 p-4">
                      ...
                      <div className="mt-4 flex flex-wrap gap-2">
                        {columnKey !== "completed" ? (
                          ...
                          >
                            {columnKey === "todo" ? "Mark Complete" : "Finish"}
                          </button>
                        ) : (
                          <button
                            type="button"
        ...
        <div className="mt-8 rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">Update the selected task and save changes.</p>
            </div>
            <button
              type="button"
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2">
            <input
              ...
              onChange={(event) =>
                setEditingTask({ ...activeTask, title: event.target.value })
              }
            ...
            <input
              ...
              onChange={(event) =>
                setEditingTask({ ...activeTask, dueDate: event.target.value })
              }
              className="w-full rounded-3xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 outline-none focus:border-slate-400 focus:ring-2 focus:ring-slate-200"
            />
          </div>
          <textarea
            ...
            onChange={(event) =>
              setEditingTask({ ...activeTask, description: event.target.value })
            }
          ...
          <div className="mt-4 flex flex-wrap gap-3">
            ...
            <button
              type="button"
              onClick={() =>
                setEditingTask({ ...activeTask, status: "in_progress" })
              }
